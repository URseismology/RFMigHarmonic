...
WrapVecDoub::WrapVecDoub(const Int nn) : vvec(nn), v(vvec), n(nn/2),
mask(n-1) {validate();}

WrapVecDoub::WrapVecDoub(VecDoub &vec) : v(vec), n(vec.size()/2),
mask(n-1) {validate();}

void WrapVecDoub::validate() {if (n&(n-1)) throw("vec size must be power of 2");}
